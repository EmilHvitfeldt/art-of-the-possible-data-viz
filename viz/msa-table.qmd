...
## Credit
This page reproduces work created by **Victor Yuan** for the 2025 Posit Table
Contest: *Visualizing Conserved Regions Across Coronavirus Spike Proteins*
([contest entry](https://github.com/rich-iannone/table-contest/discussions/10)
· [source code](https://github.com/wvictor14/2025-posit-table-contest-gt-msa/)).
All credit for the original analysis and design goes to the author.
:::

```{=html}
<ul style="display: flex; list-style: none; padding: 0; gap: 20px; flex-wrap: wrap;">
  <li><a href="https://linkedin.com/in/victor2wy/" target="_blank" rel="noopener"><i
  ...
  <li><a
      ...
      target="_blank" rel="noopener"><i class="bi bi-github"></i>Source Code</a></li>
  <li><a href="https://victoryuan.com/" target="_blank" rel="noopener"><i class="bi bi-house-fill"></i>Website</a></li>
</ul>
```

::: {.callout-tip title=":crown:"}
This submission won "Best gt Table" from the [2025 Table Contest by Posit]("https://posit.co/blog/2025-table-contest-winners/)
:::
...
```{r}
# palette
palette_aa <- readr::read_csv(file.path("msa-data", "palette_amino_acids.csv"))

# msa data
FILE_MSA <- file.path("msa-data", "msa.csv")
...
create_html_vertical_bar <- function(
  ...
  div(
    style = glue(
      "width:{parent_div_width};height:{parent_div_height};margin:0;padding:0;"
    ),
    div(
      style = glue(
        "height:{100-.bar_height}%;background:transparent;margin:0;padding:0;"
      )
    ),
    div(
      style = glue(
        "height:{.bar_height}%;background:{bar_color};margin:0;padding:0;"
      )
    )
  )
}
...
base_table <- tbl_data |>
  ...
  tab_style(
    ...
    locations = list(
      cells_body(columns = contains("pos_")),
      cells_grand_summary(columns = contains("pos_"))
    )
  ) |>
  cols_width(
    1 ~ px(60),
...
base_table |>
  gt::data_color(
    columns = contains("pos_"),
    fn = apply_color_to_aa(palette = "Chemistry")
  )
```
...
This color palette is from [ggmsa](https://yulab-smu.top/ggmsa/) that groups chemically-similar amino acids together:
...
base_table |>
  gt::data_color(
    columns = pos_1:pos_21,
    fn = apply_color_to_aa(palette = "Shapely")
  ) |>
...
A color palette from the bioinformatic software [**Rasmol**](https://openrasmol.org/doc/rasmol.html#shapelycolours) is shown here, which is based on Bob Fletterick's "Shapely Models".
...
base_table |>
  gt::data_color(
    columns = pos_33:pos_80,
    fn = apply_color_to_aa(palette = "LETTER")
  ) |>
...
base_table |>
  gt::data_color(
    columns = pos_33:pos_80,
    fn = apply_color_to_aa(palette = "Hydrophobicity")
  ) |>
  annotate_rectangle(start = pos_33, end = pos_80)
```
...
base_table |>
  gt::data_color(
    columns = pos_91:pos_95,
    fn = apply_color_to_aa(palette = "Taylor")
  ) |>
...
This color palette `Taylor` is taken from the popular MSA program [Jalview](https://www.jalview.org/help/html/colourSchemes/index.html).

:::

If you enjoyed this article and want to use some of these functions yourself, check out the development of [gtseq](https://github.com/wvictor14/gtseq)

## How it is built, step by step

The tabs above vary the palette and the highlighted region, but they all share one
`base_table`. This section takes that table apart: each tab adds one piece of the
pipeline on top of the previous one, so you can see what every part contributes.
The code for each step is shown below its output.

::: {.panel-tabset}

### 1. One column per position

The alignment arrives as one row per sequence with the residues in a single
string. Splitting it into characters, numbering the positions, then pivoting wide
gives one column per alignment position (`pos_1` … `pos_95`), which is what lets
`gt` treat residues as cells.
...
`gt()` with `rowname_col` and `groupname_col` puts the virus names in the stub
and groups them by species. The 95 position columns are far too wide at their
default size, which the next steps fix.
...
Squeezing every position column to 13px and centring the text is what makes the
table read as an alignment: residues line up into vertical columns the eye can
...
m3 <- m2 |>
  tab_style(
    style = cell_text(size = "small", align = "center", indent = 0),
...
The column labels are treated as an axis: `generate_breaks()` picks every fifth
position and `cols_label_with()` blanks the rest, so the header behaves like axis
ticks instead of 95 crowded numbers.
...
`grand_summary_rows()` adds two summary rows across every position column: the
most frequent residue, and a bar whose height is that residue's frequency. The
bar is plain HTML from `create_html_vertical_bar()`, so conservation becomes a
miniature profile plot along the bottom of the alignment.
...
### 6. Annotating the domains

Spanners name the functional regions (stem helix, HR2, TM), and relabelling five
individual columns marks the critical epitope positions with asterisks and the
glycosylation sequon with an exclamation mark. `tab_options()` then tightens the
padding, which the consensus bars need in order to sit flush against each other.
This is `base_table`.
...
#| column: screen
m6 <- m5 |>
  tab_spanner(columns = pos_1:pos_21, label = "Stem helix") |>
  tab_spanner(columns = pos_33:pos_80, label = "HR2 region") |>
  tab_spanner(columns = pos_91:pos_95, label = "TM region") |>
  cols_label(
    pos_14 ~ "*",
    pos_15 ~ "",
    pos_16 ~ "*",
...
`data_color()` with `apply_color_to_aa()` maps each residue letter to its colour
in the chosen scheme. Because it is the very last step, the same `base_table` can
be recoloured by palette or restricted to a region, which is exactly what the
tabs earlier on the page do.
